"""
aIP Assist MCP Tools Unit Tests - TDD

Tests for the aIP Assist tools that provide proactive assistance to users:
- aip_get_suggestion (REQ-004-002-001)
- aip_explain_why (REQ-004-002-002)
- aip_auto_complete (REQ-004-002-003)

aIP = Artificial Intelligence Person

Test IDs:
- UT-044: Test ActionContext schema (REQ-004-001-003)
- UT-046: Test aip_get_suggestion tool (REQ-004-002-001)
- UT-047: Test aip_explain_why tool (REQ-004-002-002)
- UT-048: Test aip_auto_complete tool (REQ-004-002-003)
"""
...
from src.mcp.tools.aip_assist import (
    AIP_ASSIST_TOOLS,
    aip_assist_handlers,
...
from src.models.aip_assist import ActionContext
...
# =============================================================================
# TEST: aIP Assist Tool Definitions
# =============================================================================

class TestAipAssistToolDefinitions:
    """Test that all aIP Assist tools are properly defined."""

    def test_aip_get_suggestion_tool_exists(self):
        """Test aip_get_suggestion tool is defined."""
        tool_names = [t["name"] for t in AIP_ASSIST_TOOLS]
        assert "aip_get_suggestion" in tool_names

    def test_aip_explain_why_tool_exists(self):
        """Test aip_explain_why tool is defined."""
        tool_names = [t["name"] for t in AIP_ASSIST_TOOLS]
        assert "aip_explain_why" in tool_names

    def test_aip_auto_complete_tool_exists(self):
        """Test aip_auto_complete tool is defined."""
        tool_names = [t["name"] for t in AIP_ASSIST_TOOLS]
        assert "aip_auto_complete" in tool_names

    def test_all_tools_have_required_fields(self):
        """Test all tools have name, description, and inputSchema."""
        for tool in AIP_ASSIST_TOOLS:
...
# =============================================================================
# TEST: aip_get_suggestion Tool (REQ-004-002-001 / UT-046)
# =============================================================================

class TestAipGetSuggestionTool:
    """Unit tests for aip_get_suggestion MCP tool."""
    ...
    def test_tool_schema_has_required_fields(self):
        ...
        """
        tool = next(
            (t for t in AIP_ASSIST_TOOLS if t["name"] == "aip_get_suggestion"),
            None
        )
    ...
    def test_get_suggestion_with_valid_context(self):
        ...
        WHEN aip_get_suggestion is invoked
        ...
        result = asyncio.run(handle_aip_get_suggestion(args))
    ...
    def test_get_suggestion_without_context_raises_error(self):
        ...
        with pytest.raises(ValueError, match="context is required"):
            asyncio.run(handle_aip_get_suggestion(args))
...
# =============================================================================
# TEST: aip_explain_why Tool (REQ-004-002-002 / UT-047)
# =============================================================================

class TestAipExplainWhyTool:
    """Unit tests for aip_explain_why MCP tool."""

    def test_tool_schema_has_required_fields(self):
        """
        GIVEN the aip_explain_why tool definition
        WHEN we inspect its schema
        THEN it must have name, description, and inputSchema
        """
        tool = next(
            (t for t in AIP_ASSIST_TOOLS if t["name"] == "aip_explain_why"),
            None
        )

        assert tool is not None, "aip_explain_why tool not found"
        assert "name" in tool
        assert "description" in tool
        assert "inputSchema" in tool
    ...
    def test_tool_requires_action_and_context(self):
        """
        GIVEN the aip_explain_why tool
        WHEN we check its input schema
    ...
    def test_explain_why_with_valid_inputs(self):
        ...
        WHEN aip_explain_why is invoked
        ...
        result = asyncio.run(handle_aip_explain_why(args))
    ...
    def test_explain_why_without_action_raises_error(self):
        ...
        with pytest.raises(ValueError, match="action is required"):
            asyncio.run(handle_aip_explain_why(args))
...
# =============================================================================
# TEST: aip_auto_complete Tool (REQ-004-002-003 / UT-048)
# =============================================================================

class TestAipAutoCompleteTool:
    """Unit tests for aip_auto_complete MCP tool."""

    def test_tool_schema_has_required_fields(self):
        """
        GIVEN the aip_auto_complete tool definition
        WHEN we inspect its schema
        THEN it must have name, description, and inputSchema
        """
        tool = next(
            (t for t in AIP_ASSIST_TOOLS if t["name"] == "aip_auto_complete"),
            None
        )

        assert tool is not None, "aip_auto_complete tool not found"
        assert "name" in tool
        assert "description" in tool
        assert "inputSchema" in tool
    ...
    def test_tool_requires_partial_input_and_field_type(self):
        """
        GIVEN the aip_auto_complete tool
        WHEN we check its input schema
    ...
    def test_auto_complete_with_valid_inputs(self):
        ...
        WHEN aip_auto_complete is invoked
        ...
        result = asyncio.run(handle_aip_auto_complete(args))
    ...
    def test_auto_complete_without_partial_input_raises_error(self):
        ...
        with pytest.raises(ValueError, match="partial_input is required"):
            asyncio.run(handle_aip_auto_complete(args))
...
class TestAipAssistToolsRegistration:
    """Tests that aIP Assist tools are properly registered."""

    def test_all_aip_assist_tools_registered(self):
        """
        GIVEN the MCP tools registry
        WHEN we get all tools
        THEN aIP Assist tools should be present
        """
    ...
    def test_all_aip_assist_handlers_registered(self):
        """
        GIVEN the MCP tool handlers
        WHEN we get all handlers
        THEN aIP Assist handlers should be present
        """
    ...
    def test_handlers_are_callable(self):
        ...
        assert callable(handle_aip_get_suggestion)
        assert callable(handle_aip_explain_why)
        assert callable(handle_aip_auto_complete)
