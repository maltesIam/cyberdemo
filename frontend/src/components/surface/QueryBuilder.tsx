  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="w-14 flex-shrink-0">
        {!isFirst ? (
          <select
            ...
            className="w-full px-1 py-1 bg-tertiary border border-primary rounded text-[11px] text-cyan-300 font-mono focus:outline-none focus:border-cyan-500"
          >
          ...
          <span className="text-[11px] text-tertiary font-mono pl-1">WHERE</span>
        )}
      </div>
      ...
      <select
        ...
        className="flex-1 min-w-0 px-2 py-1 bg-tertiary border border-primary rounded text-[11px] text-primary focus:outline-none focus:border-cyan-500"
      >
        {FIELDS.map((f) => (
      ...
      <select
        ...
        className="w-24 flex-shrink-0 px-2 py-1 bg-tertiary border border-primary rounded text-[11px] text-primary focus:outline-none focus:border-cyan-500"
      >
        {operators.map((op) => (
      ...
      {(fieldDef?.options?.length ?? 0) > 0 ? (
        ...
        <input
          ...
          onChange={(e) => onChange({ ...condition, value: e.target.value })}
          placeholder="Value..."
          className="flex-1 min-w-0 px-2 py-1 bg-tertiary border border-primary rounded text-[11px] text-primary placeholder-gray-500 focus:outline-none focus:border-cyan-500"
        />
      )}
      ...
      <button
        onClick={onRemove}
        className="p-1 text-tertiary hover:text-red-400 transition-colors flex-shrink-0"
...
export function QueryBuilder({ isOpen, onClose, onApply }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-secondary border border-primary rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col animate-query-builder-in">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-primary">
          <div className="flex items-center gap-2">
            <svg
            ...
            <h2 className="text-primary font-semibold">Advanced Query Builder</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {/* Preset queries */}
          <div>
            <span className="text-[11px] text-secondary uppercase tracking-wider mb-2 block">
              Presets
            </span>
            <div className="flex flex-wrap gap-1.5">
              {PRESET_QUERIES.map((preset) => (
                <button
                  ...
                  className="px-2.5 py-1 bg-tertiary border border-primary rounded text-[11px] text-secondary hover:bg-tertiary hover:text-primary transition-colors"
                >
                  {preset.name}
                </button>
              ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {savedQueries.map((sq, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-1 bg-tertiary border border-primary rounded overflow-hidden"
                  >
                    <button
                      onClick={() => handleLoadPreset(sq)}
                      className="px-2.5 py-1 text-[11px] text-cyan-300 hover:bg-tertiary transition-colors"
                    >
                      {sq.name}
                    </button>
                    <button
                      onClick={() => handleDeleteSaved(idx)}
                      className="px-1.5 py-1 text-tertiary hover:text-red-400 hover:bg-tertiary transition-colors"
                    >
                      <svg
                        className="w-3 h-3"
                        fill="none"
          ...
          <div className="space-y-2">
            <span className="text-[11px] text-secondary uppercase tracking-wider block">
              Conditions
            </span>
          ...
          {/* Query preview */}
          <div>
            <span className="text-[11px] text-secondary uppercase tracking-wider mb-1 block">
              Query Preview
            </span>
            <div className="px-3 py-2 bg-primary border border-primary rounded-lg font-mono text-sm text-cyan-300 min-h-[36px]">
              {queryPreview || (
                <span className="text-tertiary italic">Add conditions to build query...</span>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3 border-t border-primary">
          <div className="flex items-center gap-2">
            <button
              onClick={handleClear}
            ...
            {showSave ? (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="px-2 py-1 bg-tertiary border border-primary rounded text-xs text-primary placeholder-gray-500 focus:outline-none focus:border-cyan-500 w-40"
                ...
                <button
                  ...
                  className="px-2 py-1 bg-cyan-600 text-primary rounded text-xs disabled:opacity-50 hover:bg-cyan-500 transition-colors"
                >
                  Save
                </button>
                <button
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs text-secondary hover:text-cyan-400 transition-colors"
              >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-4 py-1.5 bg-tertiary text-secondary rounded-lg text-xs hover:bg-tertiary transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleApply}
              className="px-4 py-1.5 bg-cyan-600 text-primary rounded-lg text-xs font-medium hover:bg-cyan-500 transition-colors"
            >
              Apply Query
            </button>
          </div>
